...
import { Link } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { Phone, Mail, MapPin, Instagram, Facebook, ArrowRight, Send, CheckCircle, Wrench, Sofa, Bed, Armchair } from 'lucide-react';
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          {/* BRAND & NEWSLETTER - Updated for Prime Furniture */}
          <motion.div variants={itemVariants} className="lg:col-span-4">
            <motion.h3 
              className="text-2xl font-bold mb-6 tracking-tight"
              whileHover={{ x: 5 }}
            >
              Prime <span className="text-[#C9A03D]">Furniture</span>
            </motion.h3>
            <motion.p 
              className="text-gray-400 mb-8 leading-relaxed text-sm max-w-sm"
              variants={itemVariants}
            >
              Premium furniture, custom mattresses, and expert repair services in Vasai West. Trusted since 2008 with 4.7★ rating from 84+ satisfied customers.
            </motion.p>
          ...
          {/* NAVIGATION - Updated for Prime Furniture */}
          <motion.div variants={itemVariants} className="lg:col-span-2">
            <motion.h4 
              className="text-[10px] font-black uppercase tracking-[0.3em] text-[#C9A03D] mb-8"
              whileHover={{ x: 5 }}
          ...
          {/* SERVICES - Updated for Prime Furniture */}
          <motion.div variants={itemVariants} className="lg:col-span-3">
            <motion.h4 
              className="text-[10px] font-black uppercase tracking-[0.3em] text-[#C9A03D] mb-8"
              whileHover={{ x: 5 }}
            ...
            <ul className="space-y-4 text-sm">
              {['Sofas & Couches', 'Custom Mattresses', 'Office Chairs', 'Expert Repairs', 'Furniture Restoration'].map((service, idx) => (
                <motion.li 
                  key={service}
                  className="text-gray-400 cursor-pointer"
                  initial={{ opacity: 0.7 }}
          ...
          {/* CONTACT INFO - Updated for Prime Furniture */}
          <motion.div variants={itemVariants} className="lg:col-span-3">
            <motion.h4 
              className="text-[10px] font-black uppercase tracking-[0.3em] text-[#C9A03D] mb-8"
              whileHover={{ x: 5 }}
            ...
            <ul className="space-y-6">
              {[
                { icon: MapPin, text: "Stella Rd, Navghar Manikpur, Manickpur, Vasai West, Maharashtra 401202", isMultiLine: true },
                { icon: Phone, text: "+91 89764 23189", href: "tel:+918976423189" },
                { icon: Mail, text: "info@primefurniture.com", href: "mailto:info@primefurniture.com" }
              ].map((item, idx) => (
                <motion.li 
                  key={idx}
                  className={`flex items-start gap-4 ${item.isMultiLine ? '' : 'items-center'}`}
            ...
            {/* Rating Badge - New for Prime Furniture */}
            <motion.div 
              className="mt-6 flex items-center gap-3 bg-white/5 rounded-full px-4 py-2 border border-white/10"
              whileHover={{ scale: 1.02 }}
            >
              ...
              <span className="text-gray-400 text-xs">84+ Reviews</span>
              <div className="w-px h-4 bg-white/20"></div>
              <span className="text-gray-400 text-xs">Since 2008</span>
            </motion.div>
          </motion.div>
        </motion.div>

        {/* BOTTOM BAR - Updated for Prime Furniture */}
        <motion.div 
          variants={fadeInUp}
          className="border-t border-white/5 pt-10 flex flex-col md:flex-row justify-between items-center gap-6"
        >
          <motion.p 
            ...
            whileHover={{ color: "#C9A03D" }}
          >
            © {new Date().getFullYear()} Prime Furniture. Quality Since 2008.
          </motion.p>
          
          <div className="flex items-center gap-8">
            <div className="flex gap-4">
              {[
                { icon: Instagram, href: "https://instagram.com/primefurniture", label: "Follow us on Instagram" },
                { icon: Facebook, href: "https://facebook.com/primefurniture", label: "Follow us on Facebook" }
              ].map((social, idx) => (
                <motion.a
                  key={idx}
                  href={social.href}
